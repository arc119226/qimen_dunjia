...
This project uses **full source + bundled dist** release model:
...
│   ├── qimen.standalone.min.js # IIFE (~337KB, lunar-javascript included)
│   └── API.md                 # Bundle usage documentation
├── index.html                 # Web demo
├── index.js, qimen.js, ...    # Source files
├── package.json
└── README.md
```
...
| Need | Use |
|------|-----|
| Browser `<script>` tag | `dist/qimen.standalone.min.js` |
| Node.js / Bundler | `dist/qimen.min.js` + `npm install lunar-javascript` |
| Development / Learning | Source files |

### Creating a Release
```bash
npm run build && npm run build:standalone  # Build dist files
# Create zip with dist/ + source files
